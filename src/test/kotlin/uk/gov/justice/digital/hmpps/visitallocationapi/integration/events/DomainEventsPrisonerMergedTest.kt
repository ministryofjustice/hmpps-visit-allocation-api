...
import uk.gov.justice.digital.hmpps.visitallocationapi.service.listener.events.handlers.DomainEventHandlerRegistry.Companion.PRISONER_MERGED_EVENT_TYPE
...
class DomainEventsPrisonerMergedTest : EventsIntegrationTestBase() {
  ...
  fun `when domain event prisoner merged is found, then prisoner balance is synced`() {
    ...
    val domainEvent = createDomainEventJson(
      PRISONER_MERGED_EVENT_TYPE,
      createPrisonerMergedAdditionalInformationJson(prisonerId = prisonerId, removedPrisonerId = removedPrisonerId),
    )
    val publishRequest = createDomainEventPublishRequest(PRISONER_MERGED_EVENT_TYPE, domainEvent)

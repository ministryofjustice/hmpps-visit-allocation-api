...
class VisitAllocationByPrisonJobSqsTest : EventsIntegrationTestBase() {
  ...
  fun `when visit allocation job run for a prison then processMessage is called and visit orders are created for convicted prisoners`() {
    ...
    assertVisitOrderHistory(visitOrderHistoryList[2], prisonerId = prisoner3.prisonerId, comment = null, voBalance = 3, pvoBalance = 2, userName = "SYSTEM", type = VisitOrderHistoryType.VO_AND_PVO_ALLOCATION, attributes = mapOf(INCENTIVE_LEVEL to "ENH2"))
  }
  ...
  fun `when visit allocation job run for a prison with prisoners in a negative balance, then processMessage is called and negative visit orders are repaid for convicted prisoners`() {
    ...
    assertNegativeVisitOrdersAssignedBy(negativeVisitOrders, prisoner1.prisonerId, VisitOrderType.VO, NegativeVisitOrderStatus.REPAID, NegativeRepaymentReason.ALLOCATION, 1)
    ...
    assertNegativeVisitOrdersAssignedBy(negativeVisitOrders, prisoner1.prisonerId, VisitOrderType.PVO, NegativeVisitOrderStatus.REPAID, NegativeRepaymentReason.ALLOCATION, 1)
    ...
    assertNegativeVisitOrdersAssignedBy(negativeVisitOrders, prisoner2.prisonerId, VisitOrderType.VO, NegativeVisitOrderStatus.REPAID, NegativeRepaymentReason.ALLOCATION, 1)
    assertNegativeVisitOrdersAssignedBy(negativeVisitOrders, prisoner2.prisonerId, VisitOrderType.PVO, NegativeVisitOrderStatus.REPAID, NegativeRepaymentReason.ALLOCATION, 1)
    ...
    val visitOrderHistoryList = visitOrderHistoryRepository.findAll()
    assertThat(visitOrderHistoryList.size).isEqualTo(2)
    assertVisitOrderHistory(visitOrderHistoryList[0], prisonerId = prisoner1.prisonerId, comment = null, voBalance = -2, pvoBalance = -1, userName = "SYSTEM", type = VisitOrderHistoryType.VO_AND_PVO_ALLOCATION, attributes = mapOf(INCENTIVE_LEVEL to "STD"))
    assertVisitOrderHistory(visitOrderHistoryList[1], prisonerId = prisoner2.prisonerId, comment = null, voBalance = 2, pvoBalance = 1, userName = "SYSTEM", type = VisitOrderHistoryType.VO_AND_PVO_ALLOCATION, attributes = mapOf(INCENTIVE_LEVEL to "ENH2"))
  }
  ...
  fun `when visit allocation job run for a prison then processMessage is called and visit orders are expired for convicted prisoners`() {
    ...
    assertThat(visitOrderHistoryList.size).isEqualTo(4)
    assertVisitOrderHistory(visitOrderHistoryList[0], prisonerId = prisoner1.prisonerId, comment = null, voBalance = 1, pvoBalance = 0, userName = "SYSTEM", type = VisitOrderHistoryType.VO_ALLOCATION, attributes = mapOf(INCENTIVE_LEVEL to "STD"))
    assertVisitOrderHistory(visitOrderHistoryList[1], prisonerId = prisoner2.prisonerId, comment = null, voBalance = 26, pvoBalance = 0, userName = "SYSTEM", type = VisitOrderHistoryType.VO_EXPIRATION, attributes = emptyMap())
    assertVisitOrderHistory(visitOrderHistoryList[2], prisonerId = prisoner3.prisonerId, comment = null, voBalance = 3, pvoBalance = 4, userName = "SYSTEM", type = VisitOrderHistoryType.VO_AND_PVO_ALLOCATION, attributes = mapOf(INCENTIVE_LEVEL to "ENH2"))
    assertVisitOrderHistory(visitOrderHistoryList[3], prisonerId = prisoner3.prisonerId, comment = null, voBalance = 3, pvoBalance = 2, userName = "SYSTEM", type = VisitOrderHistoryType.PVO_EXPIRATION, attributes = emptyMap())
  }
  ...
  fun `when visit allocation job run for a prison and processing for one of the prisoners fails with a 404 error the process continues`() {
    ...
    assertVisitOrderHistory(visitOrderHistoryList[2], prisonerId = prisoner4.prisonerId, comment = null, voBalance = 3, pvoBalance = 2, userName = "SYSTEM", type = VisitOrderHistoryType.VO_AND_PVO_ALLOCATION, attributes = mapOf(INCENTIVE_LEVEL to "ENH2"))
  }
  ...
  fun `when visit allocation job runs, old accumulated VOs may expire making way for new available VOs`(
    availableVoCreateCount: Int,
    accumulatedVoCreateCount: Int,
    incentiveAmount: Int,
    expectedTotalVoCount: Int,
    expectedFinalAvailableVoCount: Int,
    expectedFinalAccumulatedVoCount: Int,
    expectedFinalExpiredVoCount: Int,
  ) {
    // Given - message sent to start allocation job for prison
    val sendMessageRequestBuilder = SendMessageRequest.builder().queueUrl(prisonVisitsAllocationEventJobQueueUrl)
    val allocationJobReference = "job-ref"
    ...
    visitOrderAllocationPrisonJobRepository.save(
      VisitOrderAllocationPrisonJob(
        allocationJobReference = allocationJobReference,
        prisonCode = PRISON_CODE,
      ),
    )

    // Maximum balance for prisoner1
    entityHelper.createPrisonerDetails(
      PrisonerDetails(prisonerId = prisoner1.prisonerId, LocalDate.now().minusDays(14), null),
    )
    entityHelper.createAndSaveVisitOrders(
      prisoner1.prisonerId,
      VisitOrderType.VO,
      VisitOrderStatus.AVAILABLE,
      LocalDateTime.now(),
      availableVoCreateCount,
    )
    entityHelper.createAndSaveVisitOrders(
      prisoner1.prisonerId,
      VisitOrderType.VO,
      ...
      LocalDateTime.now(),
      accumulatedVoCreateCount,
    )

    // When
    val convictedPrisoners = listOf(prisoner1)
    prisonerSearchMockServer.stubGetConvictedPrisoners(PRISON_CODE, convictedPrisoners)
    prisonerSearchMockServer.stubGetPrisonerById(
      prisonerId = prisoner1.prisonerId,
      createPrisonerDto(
        prisonerId = prisoner1.prisonerId,
        prisonId = PRISON_CODE,
        inOutStatus = "IN",
        lastPrisonId = PRISON_CODE,
      ),
    )
    ...
    incentivesMockServer.stubGetAllPrisonIncentiveLevels(
      prisonId = PRISON_CODE,
      listOf(PrisonIncentiveAmountsDto(visitOrders = incentiveAmount, privilegedVisitOrders = 0, levelCode = "STD")),
    )
    ...
    // Then
    assertThat(visitOrders.size).isEqualTo(expectedTotalVoCount)

    assertVisitOrdersAssignedBy(
      visitOrders,
      prisoner1.prisonerId,
      VisitOrderType.VO,
      VisitOrderStatus.AVAILABLE,
      expectedFinalAvailableVoCount,
    )
    assertVisitOrdersAssignedBy(
      visitOrders,
      prisoner1.prisonerId,
      VisitOrderType.VO,
      VisitOrderStatus.ACCUMULATED,
      expectedFinalAccumulatedVoCount,
    )
    assertVisitOrdersAssignedBy(
      visitOrders,
      prisoner1.prisonerId,
      VisitOrderType.VO,
      VisitOrderStatus.EXPIRED,
      expectedFinalExpiredVoCount,
    )
    ...
    val visitOrderAllocationPrisonJobs = visitOrderAllocationPrisonJobRepository.findAll()
    assertVisitOrderAllocationPrisonJob(
      visitOrderAllocationPrisonJobs[0],
      null,
      convictedPrisoners = 1,
      processedPrisoners = 1,
      failedOrSkippedPrisoners = 0,
    )

    val visitOrderHistoryList = visitOrderHistoryRepository.findAll()
    assertThat(visitOrderHistoryList.size).isNotEqualTo(0)
  }
  ...
  private fun assertNegativeVisitOrdersAssignedBy(visitOrders: List<NegativeVisitOrder>, prisonerId: String, type: VisitOrderType, status: NegativeVisitOrderStatus, repaidReason: NegativeRepaymentReason?, total: Int) {
